...
export async function GET(request: Request) {
  try {
    ...
    // Regular users only see their own parcel
    if (!isAdmin && userId) {
      where.userId = userId;
    }

    const parcelRequests = await (prisma as any).parcel.findMany({
      where,
    ...
    return NextResponse.json(parcelRequests);
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    const parcelRequest = await (prisma as any).parcel.create({
      data: {
