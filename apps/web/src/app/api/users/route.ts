  ...
  try {
    ...
    if (currentUserRole === "ADMIN") {
      ...
      whereClause = {
        ...
        role: {
          name: {
            in: ["ADMIN", "ORG_ADMIN", "OPERATOR", "AGENT", "USER"],
          },
        },
      };
    }

    // Apply specific role filter if provided from UI
    if (roleFilter) {
      whereClause.role = {
        name:
          roleFilter === "ADMIN" ? { in: ["ADMIN", "ORG_ADMIN"] } : roleFilter,
      };
    }
    ...
    // Standardize response for consumption by useUsers which expects .data
...
export async function POST(request: Request) {
  try {
    ...
    if (
      ...
      !["ADMIN", "OPERATOR", "AGENT", "USER"].includes(roleName)
    ) {
      return NextResponse.json(
        { error: "Forbidden role assignment" },
        { status: 403 },
      );
    }
    ...
    if (roleName === "OPERATOR" || roleName === "AGENT") {
      ...
      isFirstLogin = true;
      console.log(
        `[AUTH] Created ${roleName} - Phone: ${phone} - OTP: ${loginCode}`,
      );
    }
    ...
    let newStationId =
      currentUserRole === "ADMIN" ? currentStationId : stationId;
