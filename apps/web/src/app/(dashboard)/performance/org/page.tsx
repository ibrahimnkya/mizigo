...
async function getOrgPerformanceData() {
  // 1. Overall Stats
  const totalRequests = await prisma.parcel.count();
  const approvedRequests = await prisma.parcel.count({
    where: { status: "APPROVED" },
  });
  const pendingRequests = await prisma.parcel.count({
    where: { status: "PENDING" },
  });
  const rejectedRequests = await prisma.parcel.count({
  ...
  const avgApprovalTime =
    approvals.length > 0
      ? (totalTimeMs / approvals.length / (1000 * 60)).toFixed(1)
      : "0";

  // 3. Performance by Station (Volume)
  const volumeByStationRaw = await prisma.parcel.groupBy({
    by: ["fromAddress"],
    _count: { id: true },
  ...
  const weeklyRequests = await prisma.parcel.findMany({
  ...
  return {
    ...
    volumeByStation: volumeByStationRaw.map((v) => ({
      name: v.fromAddress,
      count: v._count.id,
    })),
    topPerformers: topPerformersRaw.map((p) => ({
      ...
      approvals: p._count.approvedParcels,
    })),
    weeklyVolume,
  };
}
...
export default async function OrgPerformancePage() {
      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <p className="text-slate-500 mt-2">
            You do not have permission to view organization performance.
          </p>
        </div>
      </div>
    );
  ...
  const stats = [
    {
      label: "Org Avg. Approval",
      value: `${data.avgApprovalTime}m`,
      sub: "Target: <60m",
      icon: Timer,
      accent: "#10b981",
    },
    {
      ...
      sub: "Total parcel handled",
      icon: BarChart3,
      accent: "#6366f1",
    },
    {
      label: "Approval Rate",
      value: `${data.totalRequests > 0 ? ((data.approvedRequests / data.totalRequests) * 100).toFixed(1) : 0}%`,
      ...
      <div className="max-w-[1520px] mx-auto px-6 lg:px-10 py-10 flex flex-col gap-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-400 font-medium mt-0.5">
              Global metrics, station volume, and team productivity scores.
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="bg-white border border-slate-100 rounded-[10px] px-4 py-2 shadow-sm">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map((s) => (
            <div
              key={s.label}
              className="bg-white rounded-[10px] border border-slate-100 p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04)] flex flex-col gap-4"
            >
              <div
        ...
        <div className="grid gap-6 lg:grid-cols-12">
              ...
              <div className="flex items-end justify-between h-48 gap-4">
                {data.weeklyVolume.map((d) => {
                  ...
                  const approvedHeight =
                    (d.approved / Math.max(d.total, 1)) * 100;
                  return (
                    <div
                      key={d.day}
                    ...
                    >
                      <div className="w-full flex flex-col justify-end h-full relative">
                        {/* Total Bar */}
                        <div
                          className="w-full bg-slate-50 rounded-t-[4px] relative overflow-hidden flex flex-col justify-end"
                          style={{ height: `${totalHeight}%` }}
                        >
                          {/* Approved Part */}
                          <div
                            className="w-full bg-indigo-500/90 rounded-t-[4px] transition-all duration-500"
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="h-1.5 w-16 bg-slate-50 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${(s.count / data.totalRequests) * 100}%`,
                          }}
                        />
                      </div>
            ...
            <div className="bg-white rounded-[10px] border border-slate-100 shadow-[0_1px_3px_rgba(0,0,0,0.04)] overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-50 flex items-center gap-2.5">
                <div className="w-7 h-7 bg-emerald-50 rounded-[10px] flex items-center justify-center">
                  <Users
                    size={13}
