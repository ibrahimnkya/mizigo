  ...
  return (
    <div className="relative space-y-0 pb-2">
      ...
      {activities.map((item, idx) => (
        <div
          key={item.id}
          className="relative pl-12 pr-2 py-4 hover:bg-slate-50/50 transition-all group rounded-[10px]"
        >
          {/* Glowing indicator */}
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center justify-between">
              <h4 className="font-bold text-slate-800 text-[13px] tracking-tight group-hover:text-indigo-600 transition-colors">
                Parcel #{item.id.slice(-6).toUpperCase()}
              </h4>
              <Badge
                variant="secondary"
                className={cn(
                  "capitalize text-[9px] font-extrabold px-1.5 py-0 rounded-[10px] border-none shadow-none",
