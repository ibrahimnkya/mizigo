...
export async function getReportStats(params: {
  ...
  const { userId, stationId, organizationId, timeframe, startDate, endDate } =
    params;
  ...
  const [received, delivered, sent, atWarehouse] = await Promise.all([
    prisma.parcel.count({ where: { ...where, status: "RECEIVED" } }),
    prisma.parcel.count({ where: { ...where, status: "COMPLETED" } }), // COMPLETED is alias for DELIVERED
    prisma.parcel.count({ where: { ...where, status: "SENT" } }),
    prisma.parcel.count({ where: { ...where, status: "AT_WAREHOUSE" } }),
  ]);
  ...
  if (organizationId || (!userId && !stationId)) {
    ...
    // This is a bit complex with current schema (ParcelRequest doesn't have stationId directly)
    // Let's approximate by fromAddress if station names are used there
    const stationVolumes = await prisma.parcel.groupBy({
      by: ["fromAddress"],
      where,
      _count: { id: true },
    ...
    topStations = stationVolumes.map(
      (s: { fromAddress: string; _count: { id: number } }) => ({
        name: s.fromAddress || "Unknown",
        volume: s._count.id,
      }),
    );

    // Top Operators
    const operatorVolumes = await prisma.parcel.groupBy({
      by: ["userId"],
      where,
      _count: { id: true },
      orderBy: { _count: { id: "desc" } },
    ...
    const operatorIds = operatorVolumes
      .map((ov: { userId: string | null }) => ov.userId)
    ...
    topOperators = operatorVolumes.map(
      (ov: { userId: string | null; _count: { id: number } }) => {
