  ...
  static async getChannels(): Promise<PaymentChannel[]> {
    try {
      ...
      if (!contentType || !contentType.includes("application/json")) {
        console.warn(
          "External API returned non-JSON content. Falling back to mock data.",
        );
      ...
      console.warn(
        "PaymentGatewayService.getChannels failed, using provided backup data:",
        error,
      );
  ...
  static async initiatePushPayment(payload: PushPaymentPayload): Promise<{
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Push payment failed: ${response.statusText} – ${errorText}`,
        );
      }
      ...
      console.warn(
        "[PaymentGateway] initiatePushPayment failed – using mock success:",
        error,
      );
      return {
        status: "SUCCESS",
