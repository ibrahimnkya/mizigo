...
class _SendParcelScreenState extends State<SendParcelScreen> {
  ...
  int _helpers = 0;
  // Step 3
  final _parcelTypeCtrl = TextEditingController();
  final _parcelSizeCtrl = TextEditingController();
  ...
  void dispose() {
    ...
    _parcelTypeCtrl.dispose(); _parcelSizeCtrl.dispose();
  ...
  Widget _buildConfirmStep() {
    return Column(
      ...
      children: [
        ...
        _InfoTile('Parcel Type', _parcelTypeCtrl.text),
        _InfoTile('Parcel Size', _parcelSizeCtrl.text),
  ...
  Future<void> _submit() async {
    ...
    final parcel = await provider.createParcel({
      'fromAddress': _fromCtrl.text.trim(),
      'toAddress': _toCtrl.text.trim(),
      'pickupType': _pickupType,
      'serviceType': _serviceType,
      'peopleNeeded': _helpers,
      'parcelType': _parcelTypeCtrl.text.trim(),
      'parcelSize': _parcelSizeCtrl.text.trim(),
    ...
    if (parcel != null && mounted) {
      context.go('/parcel/${parcel.id}/status');
    } else if (mounted) {
