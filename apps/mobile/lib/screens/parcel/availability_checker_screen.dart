...
class _AvailabilityCheckerScreenState extends State<AvailabilityCheckerScreen>
  ...
  bool _isCreatingParcel = false;
  String? _creationError;
  ParcelModel? _createdParcel;
  Timer? _timer;
  ...
  void _startChecking() {
    ...
    _timer = Timer.periodic(duration, (timer) async {
      ...
      if (_progress < 1.0) {
        ...
        setState(() {
          _isChecking = false;
          _isCreatingParcel = true;
        });
        _spinController.stop();
        _successController.forward();

        try {
          // Create the real parcel record
          final provider = Provider.of<ParcelProvider>(context, listen: false);
          final parcel = await provider.createParcel(widget.orderData);

          if (mounted) {
            setState(() {
              _isCreatingParcel = false;
              if (parcel != null) {
                _createdParcel = parcel;
              } else {
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: (_isChecking || _isCreatingParcel)
                            ? null
                            : (_createdParcel == null && _creationError == null)
                                ? null
                                : () {
                                    if (_createdParcel == null) return;

                                    if (widget.orderData['receiverPays'] == true) {
                                      // Navigate to Details/Receipt directly
                                      context.push('/bookings/recent/${_createdParcel!.id}', extra: _createdParcel);
                                    } else {
                                      // Proceed to Payment (Standard Navigation)
                                      context.push(
                                        '/parcel/${_createdParcel!.id}/payment',
                                        extra: {
                                          'amount': (_createdParcel!.amount ?? (widget.orderData['total'] as num)).toDouble(),
                                        },
                                      );
                                    }
                                  },
