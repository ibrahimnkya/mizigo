...
const operatorLogger = createServiceLogger("operators");
...
router.post("/", requirePermission("operators:create"), async (req: Request, res: Response) => {
  try {
    ...
    if (!name || !email || !phone || !password || !stationId) {
      return sendError(res, "VALIDATION_ERROR", "name, email, phone, password, and stationId are required", 400);
    }
    ...
    await prisma.user.update({
      where: { id: user.id },
      data: {
        loginCode: hashPassword(oneTimeOtp),
      },
    });
    ...
    try {
      await sendSms({
        phoneNumber: phone,
        organizationId: targetOrganizationId || null,
        message: `Welcome to Mizigo. Your one-time OTP is ${oneTimeOtp}. It expires in 24 hours.`,
      });
      smsSent = true;
    } catch (smsError) {
      operatorLogger.error("operator_otp_sms_failed", {
        operatorId: user.id,
        phone,
    ...
    await logAudit({
      ...
      action: "CREATE",
      resource: "operator",
      details: { targetUserId: user.id, stationId, organizationId: targetOrganizationId, otpSent: smsSent },
    });

    return sendSuccess(res, { ...user, onboarding: { otpSent: smsSent } }, 201);
  } catch (error: any) {
    if (error.code === "P2002") {
