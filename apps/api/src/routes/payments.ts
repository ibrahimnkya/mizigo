    ...
    const existing = await prisma.paymentProvider.findFirst({
      where: {
        OR: [
          ...
          { code: { equals: String(code), mode: "insensitive" } },
        ],
      },
    });
...
router.get("/:id", authenticate, requireTenantContext, async (req: Request, res: Response) => {
  try {
    ...
    if (req.user?.role !== "SUPER_ADMIN" && payment.cargo?.userId && req.user?.organizationId) {
...
router.post("/callback", rateLimit({ windowMs: 60 * 1000, maxRequests: 120, keyPrefix: "payments-callback" }), async (req: Request, res: Response) => {
  try {
    ...
    const resolvedStatus =
      status || (transactionstatus === "success" ? "SUCCESS" : transactionstatus ? "FAILED" : undefined);
    ...
    const payment = await prisma.payment.findFirst({
      where: {
        OR: [
          ...
          { id: additionalProperties?.parcel_transaction_uuid },
        ],
      },
      include: { cargo: true },
    });
