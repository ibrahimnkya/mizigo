...
router.post(
  ...
  requirePermission("app_versions:create"),
  async (req: Request, res: Response) => {
    ...
    if (!version || !minSupportedVersion) {
      return sendError(
        res,
        "VALIDATION_ERROR",
        "version and minSupportedVersion are required",
        400,
      );
    }
  ...
  async (req: Request, res: Response) => {
    ...
    if (!currentVersion) {
      return sendError(
        res,
        "VALIDATION_ERROR",
        "currentVersion query param is required",
        400,
      );
    }
    const versions = await prisma.appVersion.findMany({
...
router.get(
  ...
  requireTenantContext,
  async (req: Request, res: Response) => {
    const item = await prisma.appVersion.findUnique({
