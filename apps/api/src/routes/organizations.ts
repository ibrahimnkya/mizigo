...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const { name, description, supportUrl, commissionRate, commissionNotes } =
        req.body;
      if (!name) {
        return sendError(
          res,
      ...
      const organization = await prisma.organization.create({
        data: {
          ...
          commissionRate:
            commissionRate === undefined ? null : Number(commissionRate),
  ...
  try {
    ...
    if (req.user?.role !== "SUPER_ADMIN" && req.user?.organizationId !== id) {
      return sendError(
        res,
        "FORBIDDEN",
        "Cannot access this organization",
        403,
      );
    }
    ...
    try {
      ...
      if (req.user?.role !== "SUPER_ADMIN" && req.user?.organizationId !== id) {
        return sendError(
          res,
          "FORBIDDEN",
          "Cannot update this organization",
          403,
        );
      }
