    ...
    try {
      ...
      if (!isRequesterSuperAdmin && isTargetSuperAdmin) {
        return sendError(
          res,
          "FORBIDDEN",
          "Only super admin can resend OTP for super admin users",
          403,
        );
      }
      ...
      if (
        ...
        return sendError(
          res,
          "FORBIDDEN",
          "Cannot resend OTP for users outside your organization",
          403,
        );
      }
...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    // Filter out inactive or soft-deleted admins
    where.isActive = true;
    ...
    try {
      ...
      if (
        ...
        return sendError(
          res,
          "FORBIDDEN",
          "Cannot update admin from another organization",
          403,
        );
      }
    ...
    try {
      ...
      if (
        ...
        return sendError(
          res,
          "FORBIDDEN",
          "Cannot deactivate admin from another organization",
          403,
        );
      }
