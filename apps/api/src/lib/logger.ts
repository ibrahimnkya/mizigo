...
export const createUserLogger = (userId: string, username?: string | null) => {
  return logger.child(
    filterNullValues({ userId, username: username || undefined }),
  );
};
...
export const logWithContext = {
  info: (message: string, meta: Record<string, unknown> = {}) =>
    logger.info(message, filterNullValues(meta)),
  warn: (message: string, meta: Record<string, unknown> = {}) =>
    logger.warn(message, filterNullValues(meta)),
  error: (
    message: string,
    error: unknown = null,
    meta: Record<string, unknown> = {},
  ...
  debug: (message: string, meta: Record<string, unknown> = {}) =>
    logger.debug(message, filterNullValues(meta)),
};
