import crypto from "crypto";

const privateKeyPem = `-----BEGIN PRIVATE KEY-----

Xxl1aX27xQqfkVYHvccMiSR1
-----END PRIVATE KEY-----`;

const expectedPublicKey = "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCpNs5Vm1uwxrA0EEwbCACylnqzwf4kJaENxn/vbklopzyTxZjnv0eSumA7oytlwzHwQuadMS44zx0dc2xno+jNrGDYRf8Cd5iN0V1xPRhwOJeyhQhuiziw57YuUhu70gEGklb2SAFxM3DWtzecJnR17lrsCYwZ8q/0cXjbTCQUYQIDAQAB";

try {
  const pubKeyObj = crypto.createPublicKey(privateKeyPem);
  const exported = pubKeyObj.export({ type: "pkcs1", format: "der" });
  const exportedBase64 = exported.toString("base64");
  
  const spkiExported = pubKeyObj.export({ type: "spki", format: "der" });
  const spkiBase64 = spkiExported.toString("base64");

  console.log("Expected SPKI Base64:", expectedPublicKey);
  console.log("Actual SPKI Base64:  ", spkiBase64);
  console.log("Matches?", spkiBase64 === expectedPublicKey);
} catch (error: any) {
  console.error("Failed to check keys:", error.message);
}
